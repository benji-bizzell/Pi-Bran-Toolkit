...
This project uses a structured feature development pipeline powered by skills and sub-agents.

### Pipeline

```
/skill:research     → Explore, design, decompose into specs
/skill:orchestrate  → Creates task graph with dependencies (TaskCreate)
                      Spawns worker agents that self-coordinate:
                        TaskList → TaskClaim → work → TaskUpdate(completed) → loop
                      Re-run to check progress and spawn more workers
```

### Key Rules

1. **Feature artifacts live in `features/`** — organized by domain
2. **The filesystem is the state** — spec status is determined by which files exist and checklist completion
3. **Swarm pattern** — orchestrate creates task graph, workers self-coordinate via TaskClaim/TaskUpdate
4. **TDD is mandatory** — tests before implementation, always
5. **Commit discipline** — `test()` → `feat()` → `fix()` → `docs()`, never bundled
6. **QC gates at every stage** — research-qc, spec-coherence-checker, spec-qc, alignment, test-optimiser
7. **Parallel dispatch** — When spawning multiple agents for a wave, emit ALL Agent tool calls in a SINGLE response. Never call Agent one at a time. Always use `run_in_background: true` for wave agents. Foreground only for sequential gates.
...
└── {domain}/
    └── {feature-id}/
        ├── FEATURE.md              # Living documentation
        └── specs/
            └── {NN}-{spec-name}/
                ├── spec-research.md # Research (context, contracts, test plan)
...
| Agent | Role | Model |
|-------|------|-------|
| spec-coherence-checker | Validates spec internal consistency | haiku |
| checklist-completeness-validator | Ensures checklist covers all FRs | haiku |
| spec-qc | Delta check: spec vs research | sonnet |
| research-qc | Validates research outputs | sonnet |

**QC Gates (test/impl phase):**

| Agent | Role | Model |
|-------|------|-------|
| red-phase-test-validator | Validates test design quality | sonnet |
| spec-implementation-alignment | Verifies impl matches spec | sonnet |
| test-optimiser | Ensures tests provide confidence | opus |

**PR Review Agents (parallel, post-alignment):**

| Agent | Role | Model |
|-------|------|-------|
| code-reviewer | Convention compliance, bugs, quality | sonnet |
| silent-failure-hunter | Error handling, swallowed exceptions | sonnet |
| pr-test-analyzer | Test coverage quality, critical gaps | sonnet |
| comment-analyzer | Comment accuracy, stale docs | haiku |
| type-design-analyzer | Encapsulation, invariants, type safety (1-10) | sonnet |
| code-simplifier | Unnecessary complexity, redundancy | haiku |
...
Co-Authored-By: Bran <a very good bot>
EOF
)"
```
