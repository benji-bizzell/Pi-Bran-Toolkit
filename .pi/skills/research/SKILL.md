...
Commit to main
       │
       ▼
Output: Research complete, ready for /orchestrate
```
...
**Ask the user if unclear** — present what you know and ask specific questions with concrete options.

---

## Step 2: Explore the Codebase

Use the **Explore** agent for thorough investigation:
...
**Present findings first, then ask.** Use concrete options at key decision points.
...
### Module Boundary Classification

**For each dependency, classify:**

| Category | Description | Example |
|----------|-------------|---------|
| `in-process` | Direct import, same runtime | Utility functions, shared types |
| `local-substitutable` | Swappable implementation | Database client, API wrapper |
| `ports & adapters` | Behind interface, injected | External service integrations |
| `mock` | Test-only, never production | Test doubles, stubs |

**For each type/function, declare visibility:**
- **Public**: Exported via barrel file, available to other modules
- **Internal**: Implementation detail, hidden behind the barrel

This classification feeds directly into `features/module-map.md` and guides worker agents on what they can import.
...
This table enables parallel execution after research completes.
...
If this research creates a **new domain or feature**, add an entry to `features/module-map.md`:

```bash
# Add module entry with public exports, allowed/forbidden deps
# See module-map.md for format
```

For existing features, review and update the module-map entry if public exports or dependencies changed.
...
Read FEATURE.md and all spec-research.md files.
Also read: Module map: features/module-map.md (if updated)

Verify:
- Module boundaries classified for all dependencies
- Public vs internal visibility declared for all exports

Run all checks and report PASS or FAIL."
})
```
...
Report to user:
- Spec decomposition with dependency graph
- Which specs are immediately actionable (no blockers)
- **Handoff instruction:** Run `/skill:orchestrate` in a fresh session

Example output:
```
Research complete for {feature}.

Spec Decomposition:
01-foundation (no blockers - ready)
    ├── 02-data-layer (blocked by: 01)
    │       └── 03-ui (blocked by: 01, 02)
    └── 03-ui

NEXT STEP:
Start a fresh session and run:
  /skill:orchestrate features/{domain}/{feature}
...
- **Dialog at decision points** - Ask the user for real choices
...
- **Clean handoff** - Committed artifacts before moving to orchestrate
