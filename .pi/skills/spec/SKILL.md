...
1. Identify which feature and spec to work on (from argument or ask user)
...
3. **Ask the user** for any decisions needed
...
Wait for all agents to complete, then proceed.
...
**If stuck after 2 iterations:** Proceed to Step 7 which handles Andon escalation.
...
When running interactively (user invoked /skill:spec directly):
...
**If PASS:** Proceed to QC validation (Step 10)
...
**If stuck after 2 iterations:** Escalate to user.
...
**If PASS:** Proceed to commit (Step 11)

**If FAIL:**
1. Review the specific deltas listed
2. Fix inconsistencies in spec.md
3. Re-run the spec-qc agent
4. Repeat until PASS (max 2-3 iterations)

**If stuck after 2-3 iterations:** Escalate to user.
...
Add entry to Changelog table in FEATURE.md.
...
Next step: Run implement workflow or let spec-executor continue
```
